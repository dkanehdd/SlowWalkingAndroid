...
public class MenuList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new InterviewAsyncHttpRequest().execute( //1. 수락안된 인터뷰리스트 불러오기
                "http://192.168.219.116:8080/slowwalking/android/interList",
                "id="+id,
                "flag="+flag
        );
        new DiaryAsyncHttpRequest().execute( //수락된 인터뷰 리스트 불러오기
                "http://192.168.219.116:8080/slowwalking/android/diaryList",
                "id="+id,
                "flag="+flag
        );
        new DiaryListAsync().execute( //수락된 인터뷰 리스트 불러오기
                "http://192.168.219.116:8080/slowwalking/android/openCalendar",
                "id="+id,
                "flag="+flag
        );
